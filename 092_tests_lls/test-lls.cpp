...
void testList(void){
  ...
  lst.addFront(1);
  //[1]
  assert(lst.getSize()==1 && lst.size==1 && lst.head!=NULL && lst.tail==lst.head &&
	 lst.head->data==1 && lst.tail->next==NULL && lst.head->next==NULL);

  lst.addBack(2);
  //[1,2]
  assert(lst.getSize()==2 && lst.size==2 && lst.head->prev==NULL &&
	       lst.tail->data==2 && lst.head->data==1 && lst.tail->next==NULL
	       && lst.head->next->data==2 && lst.head->next->prev == lst.head);

  lst.remove(2);
  //[1]
  assert(lst.getSize() == 1 && lst.size == 1 && lst.head == lst.tail &&
         lst.head != NULL && lst.head->data == 1 && lst.tail->next==NULL && lst.head->next==NULL);
